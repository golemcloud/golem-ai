...
/// the durable websearch machinery.
#[derive(Clone)]
...
pub fn with_config_key<R>(
    ...
    match std::env::var(key) {
        ...
        Err(_) => {
            let error = SearchError::BackendError(format!("Missing config key: {key_str}"));
            fail(error)
        }
    }
}

pub fn get_config_key(key: impl AsRef<OsStr>) -> Result<String, SearchError> {
    let key_str = key.as_ref().to_string_lossy().to_string();
    std::env::var(key)
        .map_err(|_| SearchError::BackendError(format!("Missing config key: {key_str}")))
}
