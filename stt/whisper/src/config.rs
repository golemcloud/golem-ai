...
impl WhisperConfig {
    pub fn load() -> Result<Self, golem_stt::golem::stt::types::SttError> {
        use std::env;
        let api_key = env::var("OPENAI_API_KEY").map_err(|_| golem_stt::golem::stt::types::SttError::Unauthorized("missing OPENAI_API_KEY".to_string()))?;
        let endpoint = env::var("STT_PROVIDER_ENDPOINT").ok().unwrap_or_else(|| "https://api.openai.com/v1".to_string());
        ...
        let default_model = env::var("WHISPER_MODEL").ok();
