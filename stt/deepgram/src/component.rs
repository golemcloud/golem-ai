...
impl TranscriptionGuest for Component {
    ...
    fn transcribe(
        ...
        let deepgram_config = crate::config::DeepgramConfig::from_env();
        deepgram_config.common.validate_audio_size(&audio)?;

        // Direct API call - durability handled by DurableStt wrapper
        info!("Processing request {request_key}");

        let out = match wstd::runtime::block_on(client.transcribe(audio, &config, &options)) {
            Ok(o) => o,
    ...
    fn transcribe_stream(
        ...
        Err(wit_types::SttError::UnsupportedOperation(
            "Deepgram streaming requires WebSocket connection not supported in WASI environment"
                .to_string(),
        ))
    }
}
