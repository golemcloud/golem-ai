...
pub fn transcribe_impl(
    ...
    #[cfg(feature = "durability")]
    let saga: Saga<TranscriptionResult, SttError> = Saga::new("golem_stt_deepgram", "transcribe", DurableFunctionType::WriteRemote);
    ...
    let duration_seconds = if let (Some(sr), Some(ch)) = (conf.sample_rate, conf.channels) { crate::batch::compute_pcm_duration_seconds(audio.len(), sr, ch) } else { 0.0 };
