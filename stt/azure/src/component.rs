...
impl TranscriptionGuest for Component {
    ...
    fn transcribe(
        ...
        let cache_key = format!("azure:result:{request_key}");
        let lang_key = format!("azure:lang:{request_key}");
        ...
        if let Some(first_alt) = result.alternatives.first() {
            durable_store.put(&format!("azure:result:{request_key}"), &first_alt.text);
            durable_store.put(
