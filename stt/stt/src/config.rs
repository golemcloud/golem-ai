...
/// Reads an environment variable (or other config key), invoking
/// `succeed` with its value if present, or `fail` with a missing-key
/// error otherwise.
...
/// Reads an environment variable (or other config key), returning a
/// missing-key error if it is not set.
pub fn get_config_key(key: impl AsRef<OsStr>) -> Result<String, SttError> {
    let key_str = key.as_ref().to_string_lossy().to_string();
    std::env::var(key)
        .map_err(|_| SttError::InternalError(format!("Missing config key: {key_str}")))
}

/// Reads an environment variable (or other config key), returning
