...
impl Guest for AmazonLanguages {
    fn list_languages() -> Result<Vec<LanguageInfo>, SttError> {
        Ok(vec![
            LanguageInfo { code: "af-ZA".into(), name: "Afrikaans (South Africa)".into(), native_name: "Afrikaans (Suid-Afrika)".into() },
            LanguageInfo { code: "ar-AE".into(), name: "Arabic (UAE)".into(), native_name: "العربية (الإمارات)".into() },
            LanguageInfo { code: "ar-SA".into(), name: "Arabic (Saudi Arabia)".into(), native_name: "العربية (السعودية)".into() },
