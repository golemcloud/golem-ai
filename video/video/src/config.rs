...
pub fn get_config_key(key: impl AsRef<OsStr>) -> Result<String, VideoError> {
    let key_str = key.as_ref().to_string_lossy().to_string();
    std::env::var(key)
        .map_err(|_| VideoError::InternalError(format!("Missing config key: {key_str}")))
}
